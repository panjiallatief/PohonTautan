...
public class EmailController {
    ...
    @GetMapping(value = "/auth/authuser/{token}")
    public String authuser(@PathVariable(required = true) String token)  {

        Integer id = tokenRepository.getiduser(token).getId_user();
        Users personTemp = usersRepository.getById(id);
    ...
    public ResponseEntity<Map> forgotpass(HttpServletRequest request, @RequestParam String email){
        ...
        String strtoken = indexController.generetetoken(99);
    ...
    @GetMapping(value = "/pagerespas/{token}")
    public String pagerespas(@PathVariable(required = true) String token){
